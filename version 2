...
# hides inputs after user details are accepted
    ...
    income_label.pack_forget()
    income_entry.pack_forget()
...
def load_question():
    ...
    progress_label.config(
        text="Question " + str(current_question + 1) + " of " + str(len(questions))
    )
...
def back_question():
    ...
    if current_question > 0:
        ...
        previous_answer = answer_history.pop()

        # Remove the previous answer's score
        score -= questions[current_question - 1]["scores"][previous_answer]
...
def next_question():
    ...
    # Save the answer
    answer_history.append(choice.get())
